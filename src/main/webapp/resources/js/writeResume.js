...
var WriteResumeUtil = function () {
	var isAllRequired = void 0,
	    objectGenerator = void 0;

	// 전체 필수로 해야할지 boolean으로 리턴
	isAllRequired = function isAllRequired() {
		var i = void 0,
		    countRequired = void 0,
		    countObject = void 0;
	...
	objectGenerator = function objectGenerator(arr) {
		var i = void 0,
		    obj = {},
		    d = void 0;
...
var WriteResume = function () {
	var fn = void 0,
	    saveLock = void 0,
	    stepListTitle = void 0,
	    keyData = void 0,
	    _fullData = void 0,
	    _fullDataObject = void 0,
	    searchInterval = void 0,
	    lengthInterval = void 0,
	    stepData = void 0,
	    isFinalStep = void 0,
	    isFinalOK = void 0,
	    mode = void 0;
	var isItemLengthMaximized = void 0,
	    ResumeItemMap = void 0,
	    resumeItemMap = void 0;
	var hasAutoScreening = void 0; //지원자의 지원분야가 향후 서류작성완료자 자동추가 조건에 해당되는 전형이 존재할 경우, 최종제출 후 지원서 삭제가 불가능하다는 메세지를 표시합니다.
	...
	ResumeItemMap = function () {
		var ResumeItemMap = function ResumeItemMap(data) {
			var map = {};
			var f = function f(data) {
				if (data.items) {
					(Array.isArray(data.items) ? data.items : Object.keys(data.items).map(function (v) {
						return data.items[v];
		...
		ResumeItemMap.prototype = {
			getItem: function getItem(index) {
				return this.resumeItemMap[index];
			}
		};
	...
	isItemLengthMaximized = function isItemLengthMaximized($wrap, $button) {
		var bIsItemLengthMaximized = false;
		var type = $wrap.attr('data-type');
		var code = $button.closest('[data-code]').attr('data-code');
		var item = resumeItemMap.getItem(code);
		var limitLength = item.maxLimitCount;
	...
	fn = {
		...
		load: function load() {
			...
			if (!isFinalStep) {
				...
				$.ajax({
					...
					data: param,
					async: false
				}).always(Common.loading.hide).fail(Common.ajaxOnfail).done(function (x, e) {
					var nextStep = void 0,
					    i = void 0;

					_fullData = x['STEP'].items[0].items;
					stepData = x.stepList;

					// 다음 스탭이 뭔지 정하는 로직
					for (i = 0; i < stepData.length; i++) {
						if (stepData[i] === keyData.step) {
							if (stepData[i] === stepData[stepData.length - 1]) nextStep = 5;else nextStep = stepData[i + 1];
						}
					}
					...
					$('#wrapResume .limitLength').each(function () {
						// 현재 글자수 - entity code가 decode되기 전의 글자수로 불러오던 문제 해결위해 넣은 코드
						var length = $(this).siblings('textarea').text().length;
				...
				$.ajax({
					...
					data: param,
					async: false
				}).always(Common.loading.hide).fail(Common.ajaxOnfail).done(function (x) {
					_fullData = x;
					stepData = x.stepList;
		...
		templateHeader: function templateHeader(d) {
			var i = void 0,
			    t = [];
			...
			if (stepData.length > 2) {
				// 스탭 1과 최종제출만 있을 때는 안나온다.
				t.push('<nav id="tab">');
				for (i = 0; i < stepData.length; i++) {
					t.push('<div class="' + (keyData.step === stepData[i] ? 'active' : '') + '">');
					if (mode === 'write' && keyData.step < 5) t.push('<button type="submit" data-step="' + stepData[i] + '">' + stepListTitle[stepData[i] - 1] + '</button>');else t.push('<button type="button" data-type="tempSave" data-step="' + stepData[i] + '">' + stepListTitle[stepData[i] - 1] + '</button>');
					t.push('</div>');
				}
				t.push('</nav>');
			}
		...
		template: function template() {
			...
			_fullDataObject = WriteResumeUtil.objectGenerator(_fullData); // 가공된 데이터를 Array에서 Object형태로 변경(데이터를 추가할 때 필요함)
			for (i = 0, len = _fullData.length; i < len; i++) {
				d = _fullData[i];
				if (d.items && Object.keys(d.items).length || d.builder) {
					// items는 일반적인 항목, builder는 자기소개서/역량기술서
		...
		templateFinal: function templateFinal() {
			...
			if (finalCnt === _fullData.stepStatusList.length) isFinalOK = true; // 최종제출 완료된 수가 스탭 수랑 같으면 최종제출 허용
			...
			if (isFinalOK || keyData.resumeSn === 0) {
				// 지원서 미리보기 시에는 무조건 동의 서약서 노출 되어야함
				$.ajax({
					...
					async: false
				}).fail(Common.ajaxOnFail).always(Common.loading.hide).done(function (data) {
    ...
    t.push('</article>');*/
		...
		// 하단 세션 및 저장버튼 템플릿
		loadTemplateFooter: function loadTemplateFooter(nextStep) {
			var t = [];
			...
			if (!isFinalStep) {
				...
				// 최종제출일 경우
				if (isFinalOK) t.push('<div class="btnGroup"><button type="submit" class="btn btn-great">최종제출</button></div>');else t.push('<div class="btnGroup notFinal">이전 단계가 완료되지 않아 이력서를 최종제출 할 수 없습니다.</div>');
			}
			t.push('</div>');
		...
		loadTemplate: function loadTemplate(d) {
			var t = [],
			    i = void 0;
			switch (d.code) {
				case 11:
		...
		event: function event() {
			...
			$(document).on('click', 'button[data-button="copyAddress"]', function (e) {
				var $originAddress = $('div[data-row="address"]').eq(0); //항상 주소 하위 첫번째 항목에서 가져옴
				var $targetAddress = $(this).closest('div[data-row="address"]');
			...
			$(document).on('click', 'div[data-dropdown]>button', function () {
				var $dropdown = $(this).closest('div[data-dropdown]');
				if ($dropdown.hasClass('active')) $dropdown.removeClass('active');else $dropdown.addClass('active');
			...
			$(document).on('click', 'button[data-button="removeFile"]', function (e) {
				var $wrap = $(this).closest('div.row.file'),
			...
			$(document).on('click', 'button[data-button="removePhoto"]', function () {
				var $wrap = $(this).closest('div.subject.photo');
			...
			Dates.DEFINE.DEFAULT_FORMAT.YM = 'yyyy.MM', Dates.DEFINE.DEFAULT_FORMAT.MD = 'MM.dd', Dates.DEFINE.DEFAULT_FORMAT.YMD = 'yyyy.MM.dd', Dates.DEFINE.DEFAULT_FORMAT.YMDT = 'yyyy.MM.dd HH:mm', Dates.DEFINE.DEFAULT_FORMAT.TIME = 'HH:mm', Dates.init({}, 'birthday', { max: new Date().getFullYear() - 16 + '/12/31' }, 'future', { abbr: 'future' }, 'past', { abbr: 'past' }, 'college', { view: new Date().getFullYear() + '/03/01', iart: ['-1학기', '0/-6/0', '+1학기', '0/+6/0', '3학기', '1/6/0', '4학기', '2/0/0', '5학기', '2/6/0', '6학기', '3/0/0', '7학기', '3/6/0', '8학기', '4/0/0'] }, 'highschool', { view: new Date().getFullYear() + '/03/01', iart: ['2년', '1/12/0', '3년', '2/12/0'] });

			$(document).on('click', '[data-button="linkFile"]', function (e) {
				//자기가 첨부한 파일 다운로드
				var formfileUid = $(this).closest('.file').find('.inputFileUid').val();
				e.preventDefault();
			...
			$(document).on('focus', 'textarea', function (e) {
				var _this = this;
				var $limitLength = $(_this).closest('.span').find('.limitLength');
				var maxLength = $(_this).attr('data-maxlength');
		...
		downloadFile: function downloadFile(e) {
			//양식다운로드
			var formfileUid = $(this).attr('data-formFileUid');
			e.preventDefault();
			$.fileDownload('/mrs2/attachFile/downloadFile', {
				httpMethod: 'POST',
				data: 'fileUid=' + formfileUid,
		...
		removeFile: function removeFile($wrap, $spanFile, confirmOff) {
			...
			if (confirmOff) {
				Confirm('첨부한 파일을 삭제하시겠습니까?', function () {
					remove();
				});
			} else remove();
		},
		uploadFile: function uploadFile(e) {
			...
			var index = $(this).closest('div.subject').find('div.span-file').index($(this).closest('div.span-file'));

			switch (type) {
				case 'portfolioFile':
					url = '/mrs2/applicant/resume/uploadPortfolioFile';break; // 포트폴리오
				case 'transcriptFile':
					url = '/mrs2/applicant/resume/uploadTranscriptFile';break; // 성적증명서
				case 'paperFile':
				case 'researchPaper':
				case 'researchIPR':
				case 'researchWriting':
				case 'researchInvolve':
				case 'researchPresent':
					{
						//연구실적
				...
				case 'project':
					url = '/mrs2/applicant/resume/uploadProjectFile';break; // 프로젝트파일첨부
				case 'paper':
					{
						// 논문
				...
				case 'researchFile':
					url = '/mrs2/applicant/resume/uploadResearchFile';break; // 연구실적 파일첨부
				case 'careerFile':
					url = '/mrs2/applicant/resume/uploadCareerFile';break; // 경력기술서
				case 'photo':
					url = '/mrs2/applicant/resume/uploadPictureFile';break; // 사진
				case 'etcFile':
					{
						//기타 서류
			...
			$.ajax({
				...
				contentType: false,
				beforeSend: Common.loading.show
			}).always(Common.loading.hide).done(function (result) {
				var d = {},
				    $fileWrapper = void 0;
				d.fileUid = result;
		...
		submit: function submit(e) {
			...
			if (mode === 'write') fn.save(type, step);else if (mode === 'preview') fn.locationSubmit('/mrs2/applicant/resume/writeResume', step);
		},
		getFormData: function getFormData() {
			//지원자가 작성한 폼 데이터를 얻는 함수
			var param = [];
			$('#frm [name]').each(function () {
				...
				if (name !== lastName && !$(this).prop('disabled')) {
					// disabled 된 애들 저장안함
					switch (type) {
						case 'radio':
		...
		// 실제로 저장 후 이동하는 함수
		save: function save(saveType, step) {
			var url = void 0,
			    param = [],
			    valid = true,
			    isTempSave = false,
			    agreementLetterSn = void 0;
			...
			if (!isFinalStep) {
				...
				Confirm('\uC124\uC815\uC744 ' + (saveType === 'tempSave' ? '임시' : '') + '\uC800\uC7A5\uD558\uC2DC\uACA0\uC2B5\uB2C8\uAE4C?', function () {
					...
					setTimeout(function () {
						$.ajax({
							...
							Common.loading.hide();
							saveLock = false;
						}).fail(Common.ajaxOnfail).done(function (x, e) {
				...
				if (saveType === 'tempSave') {
					...
					$.ajax({ //지원자 서약서 동의 저장
						...
						Common.loading.hide();
						saveLock = false;
					}).fail(Common.ajaxOnfail).done(function (data, e) {
						$.ajax({
							type: 'post',
							dataType: 'json',
							...
							var IsNeededToReagree = !data.successYn;
							var modalAgreement = void 0,
							    funcSubmit = void 0;

							funcSubmit = function funcSubmit() {
								Confirm(hasAutoScreening ? '<strong style="color:red">최종제출한 후에는 더 이상 수정 및 삭제가 불가능</strong>하오니 반드시 확인하시고 제출하시기 바랍니다.<br>지원서 삭제를 원할경우 관리자에게 문의주세요.' : '<strong style="color:red">최종제출한 후에는 더 이상 수정이 불가능</strong><span>하오니 반드시 확인하시고 제출하시기 바랍니다.</span>', function () {
									param = {
		...
		validate: function validate() {
			...
			var i = 0,
			    j = 0;
			var recruitSectorList = [];
			var sectorRegExp = [];
			var regExp = '';
			var nullString = void 0;
			var SectorNumisMoreThan1 = void 0;
			...
			$('div[data-wrap="collegeLoop"]').each(function (index) {
				var msg = $(this).attr('data-type') === 'college' ? '대학교는 필수입력항목입니다.' : '대학원은 필수입력항목입니다.';
			...
			$('div[data-wrap="foreignExam"]').each(function () {
				var pointedIndex = void 0;
				var $wrap = $(this);
				var tmp = {};
				var languageExamCodeList = [];
				var newLanguageExamCodeList = [];
				var duplicatedLanguageCodeList = void 0,
				    duplicatedLanguageAndRegistNumList = void 0;
				...
				duplicatedLanguageCodeList = languageExamCodeList.filter(function (val, i) {
					var index = languageExamCodeList.map(function (val) {
						return val['languageExamCode'];
					}).lastIndexOf(languageExamCodeList[i]['languageExamCode']);
				...
				duplicatedLanguageAndRegistNumList = duplicatedLanguageCodeList.filter(function (val, i) {
					var arr = newLanguageExamCodeList.filter(function (v) {
			...
			if ($('div[data-recruitSector]').length) {
				...
				for (i = 0; i < $('div[data-recruitSector]').eq(0).find('select').length; i++) {
					//1지망 select몇개인지 세가지고 정규표현식만듬
					if (i === 0) sectorRegExp.push('^\\d+');else sectorRegExp.push('\\d+'); //숫자만
				}
			...
			$('input[data-limit]').each(function (index) {
				...
				var type = limit.substring(0, limit.indexOf('('));
				var tmp = void 0,
				    reg = void 0;
				var labelName = $this.attr('title') || $this.closest('div.span').find('.title').text() || $this.attr('placeholder') || '해당항목';
				...
				switch (type) {
					case 'decimal':
						...
						if (tmp.length === 1) {
							...
							if (!new RegExp(reg).test($this.val())) {
								//정수몇자리, 소수점몇자리
			...
			$('textarea[data-maxlength]').each(function (index) {
				var title = $(this).closest('div.row').find('span.title').text();
				var _this = this,
				    length = $(this).val().length,
				    max = Number($(this).attr('data-maxlength'));
			...
			$('textarea[data-minlength]').each(function (index) {
				...
				var _this = this,
				    ...
				    min = Number($(this).attr('data-minlength'));
			...
			$('input[name$=".score"], input[name$=".majorAverageScore"]').each(function () {
				var _this = this;
				var $perfectScoreElement = $(this).closest('div.span').find('input[name$=".perfectScore"],select[name$=".perfectScore"],input[name$=".majorPerfectScore"], select[name$=".majorPerfectScore"]');
				var msg = '';
			...
			$('div[data-wrap="collegeLoop"]').each(function () {
				...
				var CollegeTransferDate = null; //편입일
				var CollegeEnterDate = null; //입학일
				var tmpDate = '';

				var $dataWrap = $(this);

				var getEarlyDate = function getEarlyDate(date, standardDate) {
				...
				$dataWrap.find('div[data-loop="college"], div[data-loop="graduateSchool"]').each(function () {
					...
					if (isSelected) {
						if (!isEnteredCollege) CollegeTransferDate = getEarlyDate(tmpDate, CollegeTransferDate);else {
				...
				if ($dataWrap.find('input[name$="entranceTypeCode"]:checked').length > 0 && !hasEnteredCollege) {
					...
					Alert('입학구분이 편입의 경우 입학한 학교를 필수로 입력해야 합니다.', function () {
						var top1 = $dataWrap.find('input[name$="entranceTypeCode"]:checked').offset().top;
						var top2 = $dataWrap.closest('section[data-code]').find('button[data-button="addCollege"]').offset().top;
		...
		locationSubmit: function locationSubmit(url, step) {
			var t = [];
		...
		// 대학/대학원 삭제
		removeCollege: function removeCollege() {
			var loopName = void 0,
			    $wrap = void 0;
			...
			functionRemove = function functionRemove() {
				...
				$wrap.find('div[data-loop="' + loopName + '"]').each(function (index) {
					...
					$(this).find('[name*=' + loopName + ']').each(function () {
						var name = D.updateInputName($(this).attr('name'), index, depth);
						$(this).attr('name', name); // 업데이트 된 name을 적용
					});

					// data-rel-id 업데이트
					$(this).find('[data-rel-id*=' + loopName + ']').each(function () {
						var name = D.updateInputName($(this).attr('data-rel-id'), index, depth);
						$(this).attr('data-rel-id', name);
					});

					// data-rel-target 업데이트
					$(this).find('[data-rel-target*=' + loopName + ']').each(function () {
						var name = D.updateInputName($(this).attr('data-rel-target'), index, depth);
						$(this).attr('data-rel-target', name);
					});

					// data-dates 업데이트
					$(this).find('[data-dates*=' + loopName + ']').each(function () {
						var name = D.updateInputName($(this).attr('data-dates'), index, depth);
						$(this).attr('data-dates', name);
					});
				});
			};

			if (loopName === 'languageExam' && $loopWrap.find('.ybmValidateDone:not(.hide)').length > 0) {
				//공인외국어 인증시험일경우 reset 시 인증완료삭제
		...
		reset: function reset() {
			...
			var $wrap = void 0,
			    name = void 0,
			    languageExamSn = void 0,
			    functionReset = void 0;
			$wrap = $(this).closest('div[data-loop]');
			name = $wrap.attr('data-loop');
			functionReset = function functionReset() {
				...
				$wrap.find('[name^="' + name + '"], [data-name^="' + name + '"]').each(function () {
					...
					switch (type) {
						case 'radio':
						case 'checkbox':
							if ($(this).val()) $(this).prop('checked', false);else $(this).click(); // 값이 없는 공갈 라디오를 클릭해서 linkedForm을 비활성화
							break;
						case 'hidden':
							if ($(this).hasClass('inputFileUid')) {
								//파일인 경우
								fn.removeFile($(this).closest('div.file').find('button[data-button="removeFile"]').closest('div.row.file'), $(this).closest('div.file').find('button[data-button="removeFile"]').closest('div.row.file').closest('.span-file'), true);
							} else $(this).val('');
							break;
						default:
							$(this).val('');
			...
			if (name === 'languageExam' && $wrap.find('.ybmValidateDone:not(.hide)').length > 0) {
				//공인외국어 인증시험일경우 reset 시 인증완료삭제
		...
		// 멀티선택 드랍다운
		multiDropdown: function multiDropdown(e) {
			var name = void 0,
			    value = void 0,
			    checked = void 0,
			    isTextInput = void 0,
			    ...
			    checkMax = void 0,
			    $checkedItems = void 0;
			name = $(this).attr('name'), value = $(this).val(), checked = $(this).prop('checked'), isTextInput = D.bool($(this).attr('data-textInput')), isOthers = D.bool($(this).attr('data-others')), $wrap = $(this).closest('div[data-dropdown]');
			...
			if (!isTextInput && !isOthers && checked) {
				// 일반 메뉴를 클릭
		...
		// 멀티셀렉트
		multiSelect: function multiSelect(e) {
			var value = $(this).val();
		...
		changeRecruitField: function changeRecruitField(e) {
			...
			assoarr = _fullData[0].items['104'].recruitSectorAssoArray; //template에서 만든 연관배열을 여기서도 사용할거임
			namearr = _fullData[0].items['104'].recruitSectorDetailNames;
			currentArray = assoarr; //여기는 depth에 따라서 점점 범위가 좁혀짐
			for (i = 0; i < maxLength; i++) {
				$select = $(this).closest('div.row').find('select[data-depth="' + i + '"]');
				value = $select.val();
				title = _fullData[0].items['104'].recruitSectorHeader[i];
     ...
     for(j=0;j<Object.keys(currentArray).length;j++) {
		...
		templateSearchBox: function templateSearchBox(e) {
			var type = void 0,
			    url = void 0,
			    keyword = void 0,
			    param = void 0,
			...
			if (e.keyCode === 13) {
				...
				_this = this, type = $(this).attr('data-type'), keyword = $(this).val(), param = {}, $wrap = $(_this).closest('div.search'), $result = $wrap.find('div.searchResult');
				...
				$.ajax({
					...
					if (!len) {
						...
						searchInterval = setInterval(function () {
							if ($(_this).val()) $registBtnKeyword.html($(_this).val());else $registBtn.remove();
						}, 100);
						...
						$wrap.off('click').on('click', 'button[data-code]', function () {
							var $wrap = $(this).closest('div.span');
							...
							var languageExamSn = $(this).closest('[data-loop]').find('input[name$=".languageExamSn"]').val();

							//돔기반으로 사고하다보니 유연성이 많이 떨어지는 코드가 나왔다.
							//공인 외국어 인증 시험인지 && 그리고 인증이 완료된 항목인지를 검사해야하는 다른 코드로 변경이 필요하다.
							if ($(this).closest('[data-loop]').find('.ybmValidateDone').size() > 0 && !$(this).closest('[data-loop]').find('.ybmValidateDone').hasClass('hide')) {
								fn.resetValidatedForeignExam(languageExamSn, function () {
		...
		addSearchResult: function addSearchResult($wrap, name, code, nameValue, codeValue, elseValue) {
			var t = [],
			    index = void 0,
			    d = {};
			var funcChangeResultUI = function funcChangeResultUI() {
				// 원래는 항상 실행되는 코드블럭이었지만, 인증이 필요한 항목의 경우(현재 YBM) 인증되지 않으면 이 코드블럭이 실행되면 안되므로 함수로 묶음처리 했다.
			...
			switch (name) {
				...
				case 'languageExamName':
					...
					if (elseValue['setupcode'] === 'EXAM_CODE') {
						// setupCode가 EXAM_CODE이면 토익 인증, setupValue는 ybm에서 요구하는 코드
		...
		resetValidatedForeignExam: function resetValidatedForeignExam(languageExamSn, callback) {
		...
		resetSearchResult: function resetSearchResult(e) {
			var languageExamSn = $(this).closest('[data-loop]').find('input[name$=".languageExamSn"]').val();

			//돔기반으로 사고하다보니 유연성이 많이 떨어지는 코드가 나왔다.
			//공인 외국어 인증 시험인지 && 그리고 인증이 완료된 항목인지를 검사해야하는 다른 코드로 변경이 필요하다.
			if ($(this).closest('[data-loop]').find('.ybmValidateDone').length > 0 && !$(this).closest('[data-loop]').find('.ybmValidateDone').hasClass('hide')) {
				fn.resetValidatedForeignExam(languageExamSn, fn.resetUI.bind(this));
			} else {
				fn.resetUI.call(this);
			}
		},
		eventMouseDown: function eventMouseDown(e) {
			var type = void 0,
			    $target = void 0;
			$target = $(e.target);
			...
			if ($target.closest('div[data-dropdown]').size()) {
				//현재가 자기 켜놓은 드롭다운 빼고는 켜지게
		...
		updateStep1Data: function updateStep1Data() {
			...
			dataBasic = {};
			if (d1) {
				for (i = 0, len = d1.items.length; i < len; i++) {
					dataBasic[d1.items[i].code] = d1.items[i];
				}
			}
			...
			if (d2) {
				if (d2.items) {
					for (i = 0, len = d2.items.length; i < len; i++) {
						dataBasic[d2.items[i].code] = d2.items[i];
					}
				}
				_fullData[0].items = dataBasic; // 기본정보 내용을 array => object로 변경
			}
			...
			if (d3) {
				dataPersonal = {};
				if (d3.items) {
					for (i = 0, len = d3.items.length; i < len; i++) {
						dataPersonal[d3.items[i].code] = d3.items[i];
					}
				}
				_fullData[2].items = dataPersonal; // 인적사항 내용을 array => object로 변경
				_fullData.splice(1, 1); // 부가정보 날림
			}
		},
		updateStep2Data: function updateStep2Data() {
			var i = void 0,
			    d = void 0,
			    data = [],
			    arrSplice = [];
			...
			if (_fullData[0].code === 14 && _fullData[0].items) {
				// 학력사항인지 확인
				_fullData[0].codeName = '학력사항 추가'; // 학력사항 타이틀 변경
				for (i = 0; i < _fullData[0].items.length; i++) {
					d = _fullData[0].items[i];
					// 고등학교/대학원/대학교라면 추가 후 현 데이터는 사용하지 않는다고 표시
					if (d.code === 140 || d.code === 141 || d.code === 142) {
						data.push(d);
						delete _fullData[0].items[i];
						arrSplice.push(i);
					}
				}
				...
				for (i = arrSplice.length - 1; i >= 0; i--) {
					_fullData[0].items.splice(arrSplice[i], 1);
				}

				_fullData = data.concat(_fullData);
			}
		},
		updateStep3Data: function updateStep3Data() {
			_fullData[0].codeName = '자격/기타정보';
		},
		updateStep4Data: function updateStep4Data() {
			var i = void 0,
			    d = void 0,
			    data = [];
			for (i = 0; i < _fullData[0].items.length; i++) {
		...
		modalAddress: function modalAddress(e) {
			var interval = void 0,
			    modalOpt = void 0;
			var $parent = $(e.target).closest('.address');
			...
			modalOpt = {
				...
				btnEvent: function btnEvent() {
					var kind = '';
					if ($('.modal .external').hasClass('hide')) kind = '.internal';else kind = '.external';
				...
				callback: function callback() {
					...
					var validInterval = function validInterval(mode) {
						clearInterval(interval);
					...
					$('#modal [data-button="toExternalAddress"]').on('click', function () {
						var marginTop = ($(window).height() - '400px'.replace('px', '').replace('%', '')) / 2 + 'px';
					...
					$('#modal [data-button="toInternalAddress"]').on('click', function () {
						var marginTop = ($(window).height() - '350px'.replace('px', '').replace('%', '')) / 2 + 'px';
		...
		modalLoadResume: function modalLoadResume() {
			var modalOpt = void 0;
		...
		ybmValidate: function ybmValidate(code, d, index, callback) {
			(function (d, index) {
		...
		modalFinalSaveRejected: function modalFinalSaveRejected(data) {
			var templateModalNotAllowRegist = function templateModalNotAllowRegist(x) {
				// 중복지원 불가 안내 템플릿
				var i = void 0,
				    d = void 0,
				    t = [],
				...
				switch (data.dupApplyInspect.dayCount) {
					case 0:
						headTitle = '\'현재 접수 중인 다른 공고에 지원한 이력이 있는 경우\' ';break;
					case 30:
						headTitle = '\'최근 1개월 이내 다른 공고에 지원한 이력이 있는 경우\' ';break;
					case 90:
						headTitle = '\'최근 3개월 이내 다른 공고에 지원한 이력이 있는 경우\' ';break;
					case 180:
						headTitle = '\'최근 6개월 이내 다른 공고에 지원한 이력이 있는 경우\' ';break;
					case 365:
						headTitle = '\'최근 1년 이내 다른 공고에 지원한 이력이 있는 경우\' ';break;
				}
	...
	return {
		...
		fullDataObject: function fullDataObject() {
			return _fullDataObject;
		}
	};
}();
